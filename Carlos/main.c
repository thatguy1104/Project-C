...
void minMax(char matrix[MAX_ITERATIONS][STR_LEN], int iterations) {
    ...
    if (strcmp(matrix[0], "0001") == 0) {
        printf("Lowest number is 0001\n");
        printf("Biggest number is 0001\n");
    } else {
...
int repeatedStrings(char matrix[MAX_ITERATIONS][STR_LEN], int iterations) {
    ...
    if (n > 0) {
        printf("Repeating strings are\n");
        repeats(matrix, repetitions, n);
        happy = 0;
    } else {
        printf("No repeated strings. No conclusion can be taken");
        happy = -1;
    }
    return happy;
}
